...
const options = {
  method: "GET",
  headers: {
    "X-RapidAPI-Key": "b70bfb68eemshf000ee036273519p177f2fjsnf1fe6c4b716b",
...
const getFlightData = () => {
  ...
  fromTime = "T" + hour + ":" + d.getMinutes(); //to assign
  ...
  toTime = Number(new Date().addHours(11).getHours());
  var edit;
  ...
  if (d.getHours() >= 12) {
    ...
    toTime = "T" + toTime + ":" + d.getMinutes();
    to = fromDate + toTime;
  }

  from = fromDate + fromTime;

  // console.log(toTime, "XXXXXXXX NOW DATE XXXXXXXX");
  console.log(from);
  console.log(to);
  ...
  if (airportCode) {
    fetch(
      "https://aerodatabox.p.rapidapi.com/flights/airports/icao/" +
        ...
        "/" +
        to +
        "?withLeg=true&direction=Departure&withCancelled=true&withCodeshared=true&withCargo=true&withPrivate=true&withLocation=true",
      options
    )
      .then((res) => res.json())
      .then((data) => {
        ...
        document.getElementById("checkTimeSection").style.pointerEvents =
          "auto";
        ...
        data.departures.map((single) => {
          ...
          timeUtcflight = single.departure.scheduledTimeLocal || "Unknown";
      ...
      .catch((err) => {
        console.error(err);
        swal("Alert!", "No flight data available for this airport");
  ...
  if (airportCode) {
    fetch(
      "https://aerodatabox.p.rapidapi.com/flights/airports/icao/" +
        ...
        "/" +
        to +
        "?withLeg=true&withCancelled=true&withCodeshared=true&withCargo=true&withPrivate=true&withLocation=true",
      options
    )
      .then((res) => res.json())
      .then((data) => {
      ...
      .catch((err) => {
        console.error(err);
        swal("Alert!", "Error while fetching datadatw timsscsce");
        document.getElementById("noData").style.visibility = "visible";
      });
  } else {
    swal("Airport code not available");
